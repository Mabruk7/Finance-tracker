...
class Application(ctk.CTk):
    ...
    def create_transaction_frame(self, title, button_command, row, transactions):
        ...
        ctk.CTkLabel(frame, text=f"Add {title}", font=ctk.CTkFont(
            size=20)).grid(row=0, column=0)
        ...
        ctk.CTkButton(frame, text=f"Add {title}", command=lambda: button_command(
            category_entry, amount_entry)).grid(row=3, column=0)
    ...
    def update_plot(self):
        ...
        ax.pie([self.income, self.expense], labels=[
               'Income', 'Expense'], autopct='%1.1f%%')
        self.canvas.draw()
